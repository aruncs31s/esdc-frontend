...
const ShopCart = () => {
  const { cart, wishlist, orders, removeFromCart, updateQuantity, removeFromWishlist, addToCart, checkout, cartTotal } = useShop();
  ...
  return (
    <section style={{ minHeight: '100vh', paddingTop: '100px', paddingBottom: '60px' }}>
      <div className="container" style={{ maxWidth: '1200px' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', borderBottom: '2px solid var(--surface0)' }}>
          <button
                  ...
                  {cart.map(item => (
                    <div key={item.id} className="glass-card" style={{ padding: '1.5rem', display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ color: 'var(--subtext0)', fontSize: '0.9rem' }}>{item.description}</p>
                        <p style={{ color: 'var(--blue)', fontWeight: '700', marginTop: '0.5rem' }}>${item.price}</p>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity - 1)}
                            style={{ padding: '0.5rem 1rem', background: 'var(--surface0)', border: 'none', borderRadius: '4px', cursor: 'pointer', color: 'var(--text)' }}
                          >
                            -
                          </button>
                        ...
                        <button
                          ...
                          style={{ padding: '0.5rem', background: 'var(--red)', border: 'none', borderRadius: '4px', cursor: 'pointer', color: 'white' }}
                        >
                          <FiTrash2 />
                        </button>
                      </div>
                ...
                {wishlist.map(item => (
                  <div key={item.id} className="project-card">
                    ...
                    <div className="project-content">
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--surface0)' }}>
                        <div style={{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--blue)' }}>${item.price}</div>
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          <button onClick={() => addToCart(item)} className="btn btn-primary" style={{ padding: '10px 20px', fontSize: '0.9rem' }}>
                            Add to Cart
                          </button>
                          <button onClick={() => removeFromWishlist(item.id)} style={{ padding: '10px', background: 'var(--red)', border: 'none', borderRadius: '8px', cursor: 'pointer', color: 'white' }}>
                            <FiTrash2 />
                          </button>
                        </div>
                      </div>
                ...
                {orders.map((order, orderIndex) => {
                  ...
                  return (
                    <div key={orderIndex} className="glass-card" style={{ padding: '1.5rem' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {order.map(item => (
                          <div key={item.id} style={{ display: 'flex', gap: '1rem', alignItems: 'center', padding: '1rem', background: 'var(--surface0)', borderRadius: '8px' }}>
                            <img src={item.image} alt={item.name} style={{ width: '60px', height: '60px', objectFit: 'cover', borderRadius: '4px' }} />
                            <div style={{ flex: 1 }}>
                              <h4>{item.name}</h4>
                              <p style={{ color: 'var(--subtext0)', fontSize: '0.9rem' }}>Quantity: {item.quantity}</p>
                            </div>
                            <div style={{ fontWeight: '700', color: 'var(--blue)' }}>${(item.price * item.quantity).toFixed(2)}</div>
                          </div>
                        ))}
                      </div>
                      <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--surface0)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h3>Order Total:</h3>
