...
import { FiGithub, FiAward, FiTrendingUp, FiCalendar, FiMapPin, FiMail, FiLink, FiShoppingCart, FiHeart, FiPackage, FiCode, FiTarget, FiZap } from 'react-icons/fi';
...
const UserProfile = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      ...
      setAchievements([
        { id: 1, icon: 'target', title: 'First Steps', desc: 'Completed first challenge', color: 'var(--blue)' },
        { id: 2, icon: 'fire', title: 'On Fire', desc: '7 day streak', color: 'var(--peach)' },
        { id: 3, icon: 'star', title: 'Rising Star', desc: 'Earned 500 points', color: 'var(--yellow)' }
      ]);
  ...
  return (
    <section className="profile-page">
      <div className="container" style={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div className="profile-card" style={{ maxWidth: '100%', position: 'sticky', top: '100px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div className="profile-info-card">
            <h3 style={{ fontSize: '1.3rem', marginBottom: '24px', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          {/* Achievements */}
          <div className="profile-info-card">
            <h3 style={{ fontSize: '1.3rem', marginBottom: '24px', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: '600' }}>
              <FiAward style={{ color: 'var(--yellow)' }} /> Achievements
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {achievements.map((achievement: any) => (
                <div key={achievement.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: '10px', border: '1px solid var(--surface0)', transition: 'all 0.3s ease' }}>
                  <div style={{ 
                    width: '48px', 
                    height: '48px', 
                    display: 'flex', 
          ...
          {/* Shopping Cart */}
          <div className="profile-info-card">
            <h3 style={{ fontSize: '1.3rem', marginBottom: '24px', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between', fontWeight: '600' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <FiShoppingCart style={{ color: 'var(--blue)' }} /> Shopping Cart
              </span>
              <Link to="/cart" style={{ fontSize: '0.9rem', color: 'var(--blue)', textDecoration: 'none' }}>
                View All
              </Link>
            </h3>
            {cart.length > 0 ? (
              <>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '15px' }}>
                  {cart.slice(0, 3).map(item => (
                    <div key={item.id} style={{ display: 'flex', gap: '12px', padding: '12px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: '10px', border: '1px solid var(--surface0)' }}>
                      <img 
                        src={item.image} 
                        alt={item.name}
                      ...
                      />
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: '600', color: 'var(--text)', marginBottom: '4px' }}>{item.name}</div>
                        <div style={{ fontSize: '0.85rem', color: 'var(--subtext0)', marginBottom: '4px' }}>
                          Qty: {item.quantity}
                        </div>
                        <div style={{ fontSize: '0.9rem', color: 'var(--blue)', fontWeight: '600' }}>
                ...
                <div style={{ 
                  ...
                  <span style={{ color: 'var(--blue)', fontSize: '1.2rem' }}>${cartTotal.toFixed(2)}</span>
                </div>
              </>
            ) : (
          ...
          <div className="profile-info-card">
            <h3 style={{ fontSize: '1.3rem', marginBottom: '24px', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between', fontWeight: '600' }}>
              ...
              <Link to="/wishlist" style={{ fontSize: '0.9rem', color: 'var(--blue)', textDecoration: 'none' }}>
                View All
              </Link>
            </h3>
            {wishlist.length > 0 ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '12px' }}>
                {wishlist.slice(0, 4).map(item => (
                  <div key={item.id} style={{ position: 'relative', borderRadius: '10px', overflow: 'hidden', border: '1px solid var(--surface0)' }}>
                    <img 
                      src={item.image} 
                      alt={item.name}
          ...
          <div className="profile-info-card">
            <h3 style={{ fontSize: '1.3rem', marginBottom: '24px', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between', fontWeight: '600' }}>
              ...
              <Link to="/projects" style={{ fontSize: '0.9rem', color: 'var(--blue)', textDecoration: 'none' }}>
                View All
              </Link>
            </h3>
            {userProjects.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {userProjects.map((project: any) => (
                  <div key={project.id} style={{ display: 'flex', gap: '12px', padding: '12px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: '10px', border: '1px solid var(--surface0)' }}>
                    <img 
                      src={project.image} 
                      alt={project.title}
                    ...
                    />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: '600', color: 'var(--text)', marginBottom: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                        <span>{project.title}</span>
                        <span className={`status-badge ${project.status === 'completed' ? 'completed' : 'in-progress'}`} style={{ fontSize: '0.75rem' }}>
                          {project.status}
                        </span>
                      </div>
                      <div style={{ fontSize: '0.85rem', color: 'var(--subtext0)', marginBottom: '8px' }}>
                        {project.description}
                      </div>
          ...
          <div className="profile-info-card">
            <h3 style={{ fontSize: '1.3rem', marginBottom: '24px', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'space-between', fontWeight: '600' }}>
              ...
              <Link to="/my-products" style={{ fontSize: '0.9rem', color: 'var(--blue)', textDecoration: 'none' }}>
                View All
              </Link>
            </h3>
            {userProducts.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {userProducts.map((product: any) => (
                  <div key={product.id} style={{ display: 'flex', gap: '12px', padding: '12px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: '10px', border: '1px solid var(--surface0)' }}>
                    <img 
                      src={product.image} 
                      alt={product.name}
                      style={{ width: '80px', height: '80px', objectFit: 'cover', borderRadius: '8px' }}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: '1rem', color: 'var(--blue)', fontWeight: '600' }}>
                          ${product.price}
                        </span>
